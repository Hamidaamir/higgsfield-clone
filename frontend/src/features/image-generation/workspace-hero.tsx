const cards = [
  { rotate: "-8deg", art: "radial-gradient(80% 80% at 30% 30%, #f6b26b 0%, #3b1f0e 70%)", offset: "-30px" },
  { rotate: "-3deg", art: "radial-gradient(80% 80% at 60% 40%, #ff5a8a 0%, #2a0a1a 70%)", offset: "-10px" },
  { rotate: "3deg", art: "radial-gradient(80% 80% at 40% 60%, #9ad1ff 0%, #0b1d33 70%)", offset: "-10px" },
  { rotate: "8deg", art: "radial-gradient(80% 80% at 50% 30%, #d6ff00 0%, #1c2405 70%)", offset: "-30px" },
];
    ...
    <div className="flex flex-col items-center px-4 pt-6 text-center sm:pt-10">
      <div className="flex h-40 items-end justify-center sm:h-48" aria-hidden>
        {cards.map((card, i) => (
          <div
            key={i}
            className="h-32 w-24 rounded-2xl border border-white/20 shadow-card sm:h-40 sm:w-32"
            style={{
              backgroundImage: card.art,
              transform: `rotate(${card.rotate}) translateY(${card.offset})`,
